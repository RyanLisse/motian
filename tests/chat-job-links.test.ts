...
describe("chat job links", () => {
  it("normalizes only relative or same-origin Motian job URLs", () => {
    expect(normalizeChatJobHref(`/vacatures/${JOB_ID}`)).toBe(`/vacatures/${JOB_ID}`);
    expect(normalizeChatJobHref(`/opdrachten/${JOB_ID}`)).toBe(`/vacatures/${JOB_ID}`);
    expect(normalizeChatJobHref(`/opdracht/${JOB_ID}`)).toBe(`/vacatures/${JOB_ID}`);
    expect(
      normalizeChatJobHref(
        `https://motian.local/vacatures/${JOB_ID}?utm=ignored`,
        "https://motian.local",
      ),
    ).toBe(`/vacatures/${JOB_ID}`);
    ...
    expect(
      normalizeChatJobHref(`https://external.example/vacatures/${JOB_ID}`, "https://motian.local"),
    ).toBeNull();
  });
  ...
  it("rewrites internal job markdown links without touching external links", () => {
    const rewritten = rewriteChatJobLinks(
      [
        `Open [Bekijk vacature](/vacatures/${JOB_ID}).`,
        "Gebruik ook [de bron](https://external.example/job) als referentie.",
      ].join("\n\n"),
    );

    expect(rewritten).toContain(
      `<motian-job-link href="/vacatures/${JOB_ID}">Bekijk vacature</motian-job-link>`,
    );
  ...
  it("rewrites internal job html anchors without touching external html anchors", () => {
    ...
    expect(rewritten).toContain(
      `<motian-job-link href="/vacatures/${JOB_ID}">Bekijk opdracht</motian-job-link>`,
    );
  ...
  it("preserves inline code spans and fenced code blocks while still rewriting real links", () => {
    const rewritten = rewriteChatJobLinks(
      [
        `Open [Bekijk vacature](/vacatures/${JOB_ID}).`,
        "",
        `Inline code: \`[Bekijk vacature](/vacatures/${JOB_ID})\` en \`<a href="https://motian.nl/opdracht/${JOB_ID}">Bekijk opdracht</a>\``,
        "",
        "```md",
        `[Bekijk vacature](/vacatures/${JOB_ID})`,
        `<a href="https://motian.nl/opdracht/${JOB_ID}">Bekijk opdracht</a>`,
        "```",
      ].join("\n"),
      "https://motian.local",
    ...
    expect(rewritten).toContain(`\`[Bekijk vacature](/vacatures/${JOB_ID})\``);
    expect(rewritten).toContain(
      `\`<a href="https://motian.nl/opdracht/${JOB_ID}">Bekijk opdracht</a>\``,
    );
  ...
  it("renders internal job links as direct app anchors while external links stay on the safety path", () => {
    const html = renderToStaticMarkup(
      createElement(
        ...
        rewriteChatJobLinks(
          `Bekijk [de vacature](/vacatures/${JOB_ID}) of [de externe bron](https://external.example/job).`,
        ),
      ),
    );

    expect(html).toContain(`href="/vacatures/${JOB_ID}"`);
