...
### Quality Rules & Fighting Entropy (Cruft Management)
...
- **Avoid Entropy:** Do not leave behind commented-out code, unused variables, or temporary `console.log` statements.
- **Run Entropy Check:** Before completing work, verify `pnpm tsx scripts/harness/entropy-check.ts` (if available) passes.
...
# 4. Verification (Say No to Slop)
git status  # MUST show "up to date with origin"
pnpm exec tsc --noEmit # MUST verify no new TypeScript errors were introduced
# Agents MUST write at least one basic structural or unit test for new utility functions or services before finishing.
```
...
<!-- OMX:RUNTIME:START -->
<session_context>
**Session:** omx-1775314669992-blq4yk | 2026-04-04T14:57:50.052Z

**Codebase Map:**
  src/: env
  src/actions/: match-linking, match-review
  src/ai/: agent, analyse-data, berichten, canvas, gdpr, get-opdracht-detail, interviews, kandidaten, match-kandidaten
  src/autopilot/: analyze-evidence, schemas, journeys, capture, content-type, journey-runner
  src/cli/: commands, parse-args
  src/components/: chat-prompt-composer, conversation, message, prompt-input, suggestion, use-chat-cv-upload, posthog-provider
  src/db/: schema
  src/harness/: adapter, client, mappers, publication, queries, types, plan
  src/hooks/: use-event-source
  src/lib/: ai-models, api-cache, api-cors, api-docs, api-handler, breadcrumbs, chat-origin, crypto, cv-upload, event-bus
  src/mcp/: create-server, server, advanced-matching, analytics, batch-operations, chat-sessions, cv-operations, esco-skills, gdpr-ops
  src/schemas/: a2ui, candidate-intelligence, job, koppeling, matching, platform-catalog, scraper-analyse, settings, whatsapp
  src/services/: agent-events, ai-enrichment, applications, a...
