...
const analyseDataSchema = z.object({
  analysis: z
    .enum([
      "jobs_per_platform",
      "avg_rates",
...
export const tools = [
  {
    ...
    inputSchema: zodToJsonSchema(analyseDataSchema, { $refStrategy: "none" }),
  },
];
...
export const handlers: Record<string, (args: unknown) => Promise<unknown>> = {
  analyse_data: async (raw) => {
    const { analysis } = analyseDataSchema.parse(raw);
    ...
    switch (analysis) {
      ...
      default:
        return { error: "Onbekend analyse type" };
    }
  },
};
